...
const ROLE_TABS = ['all', 'customer', 'caterer', 'agency', 'worker', 'driver', 'home_pro', 'admin'] as const
...
export function AdminUsersPage() {
  ...
  const counts = useMemo(() => Object.fromEntries(
    ROLE_TABS.map((id) => [
      id,
      id === 'all' ? rows.length : rows.filter((u) => roleList(u).includes(id)).length,
    ]),
  ...
  const tabLabel: Record<string, string> = {
    ...
    agency: t('tabs.agencies'),
    worker: t('tabs.workers'),
    driver: t('tabs.drivers'),
    home_pro: t('tabs.homePros'),
    admin: t('tabs.admins'),
  }
  ...
  return (
    <AdminPage>
      ...
      <AdminTabs>
        {ROLE_TABS.map((id) => (
          <AdminTab key={id} active={tab === id} onClick={() => setTab(id)}>
            {tabLabel[id] || id} ({counts[id] ?? 0})
          </AdminTab>
        ))}
      </AdminTabs>
      ...
      <AdminWorkspace
        ...
        detail={(
          <div className="side-panel">
            <AdminDetailCard
              kicker={t('users.profile')}
              title={picked?.name}
              empty={t('common.selectRow')}
            >
              {picked ? (
                <>
                  <div className="meta admin-detail-roles">
                  ...
                  <div className="kv">
                    ...
                    {picked.worker_profile && (
                      <>
                        <div className="kv-row"><span>{t('tabs.workers')}</span><StatusBadge value={picked.worker_profile.status || 'active'} /></div>
                        <div className="kv-row"><span>{t('users.rating')}</span><strong>{picked.worker_profile.rating_avg || 0}</strong></div>
                        <div className="kv-row">
                          <span>{t('users.skills')}</span>
                          <strong>{(picked.worker_profile.skills || []).map((s) => s.category?.name).filter(Boolean).join(', ') || '—'}</strong>
                        </div>
                      </>
                    )}
                  </div>
